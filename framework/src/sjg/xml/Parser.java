...
public class Parser {
                ...
                switch (tagStyle) {
                    ...
                    case CLOSE_TAG:
                        if (topElement.getName().equals(name))
                            ...
                            throw new Error("Expected close of '" + topElement.getName() + "' instead of " + s2);
                        break;
                    case OPEN_CLOSE_TAG:
                        pushElement(new Element(name));
                        popElement();
    ...
    private void newProcessingInstruction(String s) {
        if (topElement != null)
            topElement.addContents(new ProcessingInstruction(s.substring(2, s.length() - 2)));
        else if (rootElement == null) header.append(s);
    }
     ...
     */
    public static Document parse(java.applet.Applet applet, String fn) {
        return parse(applet.getClass().getResourceAsStream(fn));
    }
